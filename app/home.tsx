...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Initialize i18n and get current language (non-blocking, optional)
    // Don't block app loading if i18n fails
    try {
      initI18n()
        .then(() => {
          try {
            ...
            console.error("[HOME] getLanguage error:", e);
            setCurrentLanguage("tr");
          }
        })
        .catch((error) => {
          console.error("[HOME] i18n init error:", error);
          setCurrentLanguage("tr"); // Default to Turkish on error
        });
    } catch (error) {
      console.error("[HOME] i18n init try-catch error:", error);
      setCurrentLanguage("tr");
    }
  ...
  const getProgressSteps = () => {
    ...
    return [
      ...
      { id: "treatment", label: t("home.progressTreatment"), completed: treatmentCompleted, inProgress: treatmentInProgress },
    ];
  };
      ...
      <View style={[styles.header, { backgroundColor: isPro ? "#FFFFFF" : "#F9FAFB" }]}>
        ...
        <View style={styles.headerActions}>
          ...
          <Pressable
            onPress={() => {
              try {
                const languages = SUPPORTED_LANGUAGES || ["tr", "en"];
                const languageOptions = languages.map((lang) => ({
                  text: (LANGUAGE_NAMES && LANGUAGE_NAMES[lang]) || (lang === "tr" ? "Türkçe" : "English"),
                  onPress: async () => {
                    try {
