...
export default function PatientDashboard() {
  ...
  const { clearClinic, activeClinic } = useClinicStore();
  const { t } = useLanguage();
  ...
  /** GET /api/patient/me — clinic_id + clinic embed (authoritative vs JWT). */
  const [patientMe, setPatientMe] = useState<{
  ...
  /** If JWT already shows a clinic, do not flash Find/Join while /me catches up (Profile parity). */
  const isLoadingClinicSection =
    (!patientMeSynced && !clinicMembership.jwtLinked) || refreshing;

  useEffect(() => {
    if (!__DEV__) return;
    console.log("[PATIENT_HOME][clinic]", "home_clinic_state", {
      source: "derivePatientClinicMembership",
      hasClinic: clinicMembership.hasClinic,
      jwtLinked: clinicMembership.jwtLinked,
      meLinked: clinicMembership.meLinked,
      sources: clinicMembership.sources,
      patientMeSynced,
      refreshing,
      isLoadingClinicSection,
    });
  }, [clinicMembership, patientMeSynced, refreshing, isLoadingClinicSection]);
  const linkedClinicTitle = useMemo(() => {
    const fromMe =
      patientMe &&
      (String(patientMe.clinic?.name || "").trim() ||
        String(patientMe.clinicName || "").trim() ||
        String(patientMe.branding?.clinicName || "").trim());
    if (fromMe) return fromMe;
    if (userClinicCode) return userClinicCode;
    const ac = String(activeClinic?.name || "").trim();
    if (ac) return ac;
  ...
  const refetchPatientMe = useCallback(async (tokenOverride?: string) => {
    ...
    try {
      ...
      const clinicEmbed =
        ...
        meJson.clinic.id != null &&
        String(meJson.clinic.id).trim() !== ""
          ? {
              id: String(meJson.clinic.id),
              name: String(
                meJson.clinic.name || meJson.clinic_name || brand?.clinicName || ""
              ).trim(),
              logo:
      ...
      __DEV__ &&
        console.log("[PATIENT_HOME][clinic]", "membership_refetch_patient_me", {
          ...derivePatientClinicMembership({ user, patientRecord: nextRecord }),
          cidRaw,
          clinicCode: codeTop,
        });
  ...
  useEffect(() => {
    const unsub = subscribePatientClinicMembershipInvalidation((reason) => {
      __DEV__ &&
        console.log("[PATIENT_HOME][clinic]", "cache_invalidation_refetch", reason);
  ...
  const goToClinicProfile = useCallback(() => {
        ...
        { text: t("common.cancel") || "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              await refetchPatientMe(newTok);
              invalidatePatientClinicMembership("patient_dashboard_leave");
              Alert.alert(
                t("profile.leaveClinic.successTitle") || "",
                t("profile.leaveClinic.successMsg") || "",
                [{ text: "OK" }]
              );
            } catch (err: unknown) {
              Alert.alert(
    ...
    >
      ...
      <View style={styles.ctaSection}>
        ...
        {isLoadingClinicSection ? null : !hasClinic ? (
          ...
          <View style={styles.ctaClinicRow}>
            <View style={styles.ctaClinicLeft}>
              <Image
                source={{
                  uri: (() => {
                    const a = (patientMe?.clinic?.logo || "").trim();
                    const b = (patientMe?.branding?.clinicLogoUrl || "").trim();
                    return a || b || DEFAULT_CLINIC_LOGO_URI;
                  })(),
                }}
                style={{ width: 40, height: 40, borderRadius: 8 }}
              />
              <Text style={styles.ctaClinicName} numberOfLines={2}>
                {linkedClinicTitle}
              </Text>
            </View>
            <TouchableOpacity
