...
import { patientHasClinicFromJwt } from "../../../lib/patientClinicMembership";
import {
...
export default function ProfileScreen() {
  ...
  const clinicCode = String((user as { clinicCode?: string })?.clinicCode || "").trim();
  const clinicId = String((user as { clinicId?: string })?.clinicId || "").trim();
  const hasClinic = patientHasClinicFromJwt(user);
        ...
        { text: t("common.cancel") || "Vazgeç", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              invalidatePatientClinicMembership("profile_leave_clinic");
  ...
  const joinWithCode = useCallback(async () => {
    ...
    try {
      ...
      invalidatePatientClinicMembership("profile_join_clinic");
      void refreshActiveClinicFromApi(data.token);
      setJoinModal(false);
      setJoinClinicCode('');
      setJoinReferralCode('');
  ...
  const handleLogout = () => {
    Alert.alert(t("profile.logoutTitle"), t("profile.logoutConfirm"), [
      { text: t("profile.logoutCancel"), style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: async () => {
          await signOut({ replaceTo: "/role-select" });
        },
      },
    ]);
  };
