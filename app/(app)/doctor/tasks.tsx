...
type DoctorTask = {
  ...
  /** Clinic-time HH:mm from API (same contract as doctor dashboard) */
  time?: string | null;
...
/** Calendar label for YYYY-MM-DD without shifting the civil day across device timezones. */
function ymdToLocaleLabel(ymd: string) {
...
/** Prefer server clinic-normalized fields; avoid Date→toLocaleString on UTC/zoned ISO (device TZ drift vs dashboard). */
function fmtTaskSchedule(task: DoctorTask) {
  const hm = String(task.scheduledLocalTime || task.time || "").trim();
  const ymd =
    String(task.scheduledLocalDate || "").trim() ||
    (typeof task.due_date === "string" && /^\d{4}-\d{2}-\d{2}/.test(task.due_date) ? task.due_date.slice(0, 10) : "");
  if (ymd && hm) {
    return `${ymdToLocaleLabel(ymd)} · ${hm}`;
  }
  return fmtDueLegacy(task.due_date);
}

function fmtDueLegacy(d: string | null | undefined) {
  if (!d) return "—";
  try {
    const s = String(d).trim();
    const dateOnly = /^\d{4}-\d{2}-\d{2}$/.test(s);
    const x = dateOnly ? new Date(`${s}T12:00:00Z`) : new Date(s);
    if (Number.isNaN(x.getTime())) return s;
    if (dateOnly) {
      return ymdToLocaleLabel(s);
    }
          ...
          {tasks.length === 0 ? (
            ...
            tasks.map((task) => (
              <View key={task.id} style={styles.card}>
                ...
                <Text style={styles.meta}>
                  {statusLabel(task.status)} · {fmtTaskSchedule(task)}
                </Text>
                <Pressable
